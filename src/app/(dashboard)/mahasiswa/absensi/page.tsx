...
export default function AbsensiPage() {
  ...
  // ✅ FIX: Check if time is past deadline
  ...
  const handleAbsen = async () => {
    ...
    try {
      ...
      if (isPastDeadline(batas)) {
        ...
        // ✅ FIX: Auto-set status to "alpa" when past deadline
        const { error: alpaError } = await supabase.from('presensi').upsert({
      ...
      if (selectedJadwal.wajib_foto) {
        ...
        try {
          ...
          const { data: uploaded } = await supabase.storage.from('attendance-photos').upload(
      ...
      // ✅ FIX: Geolocation opsional, tidak memblokir absen
  ...
  return (
    <div className="space-y-6">
              ...
              <CardContent className="flex items-center justify-between gap-3 p-3 sm:p-4">
                <div className="space-y-1 min-w-0">
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    {j.wajib_foto && <Badge variant="info">Wajib Foto</Badge>}
      ...
      {selectedJadwal && (
        <Card>
          <CardHeader>
            <CardTitle>Absen: {selectedJadwal.nama_kegiatan}</CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="flex items-center gap-2 text-sm">
              ...
              {geoLoading ? (
                ...
                <span className="text-xs text-orange-500">⚠️ Lokasi tidak tersedia (optional)</span>
              ) : latitude ? (
                `${latitude.toFixed(6)}, ${longitude?.toFixed(6)}`
              ) : (
                'Klik untuk dapatkan lokasi'
              )}
